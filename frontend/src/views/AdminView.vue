        ...
        >
          <div class="event-header">
            ...
            <div class="event-meta-inline">
              <span class="participant-count">{{ event.participants.length }} participants</span>
              <span class="location-count">{{ event.locations.length }} locations</span>
            </div>
          </div>
          ...
          <div class="event-summary">
            <div class="summary-section">
              <h4>📍 Locations</h4>
              <div class="tags">
                <span v-for="location in event.locations.slice(0, 3)" :key="location" class="tag">
                  {{ location }}
                </span>
                <span v-if="event.locations.length > 3" class="tag more">
            ...
            <div class="summary-section">
              <h4>👥 Organizers</h4>
              <div class="tags">
                <span v-for="organizer in event.organizers.slice(0, 3)" :key="organizer" class="tag">
                  {{ organizer }}
                </span>
                <span v-if="event.organizers.length > 3" class="tag more">
                  +{{ event.organizers.length - 3 }} more
                </span>
              </div>
            </div>
            
            <div v-if="event.participants.length > 0" class="summary-section">
              <h4>💰 Payments</h4>
          ...
          <div class="event-summary">
            <div class="summary-section">
              ...
              <div class="dates-list">
                <div v-for="date in event.dates.slice(0, 2)" :key="date" class="date-item small">
                  {{ formatDateTime(date) }}
                </div>
                <div v-if="event.dates.length > 2" class="date-item small more">
          ...
          <div class="event-actions">
            ...
            <button @click="deleteEventAction(event.id)" class="delete-btn">
              Delete
            </button>
          </div>
        </div>
...
const totalDue = (event: any): number => {
  return event.participants.reduce((sum: number, p: any) => sum + p.due_payment, 0)
}

const totalPaid = (event: any): number => {
  return event.participants.reduce((sum: number, p: any) => sum + p.paid_amount, 0)
}
